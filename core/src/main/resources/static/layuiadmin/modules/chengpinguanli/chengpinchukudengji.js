layui.define(['table', 'laydate', 'form', 'upload'], function (exports) {
    ...
    var cols = [[
        ...
        , {title: '购货单位',templet: repNull('gouhuodanwei')}
        , {title: '收货单位',templet: repNull('shouhuodanwei')}
        , {title: '联系人',templet: repNull('lianxiren')}
        , {title: '联系电话',templet: repNull('lianxidianhua')}
        , {title: '收货地址',templet: repNull('shouhuodizhi')}
        , {title: '出库类型',templet: repNull('chukuleixing.name')}
        , {title: '营销员',templet: repNull('yingxiaoyuan.xingming')}
        , {title: '申请长度', field: 'changdu',sort: true}
        , {title: '申请备注',field: 'beizhu'}
        , {title: '要求领用时间',templet: repNull('yaoqiulingyongshijian'),field: 'yaoqiulingyongshijian', sort: true}
        , {title: '仓库确认人',templet: repNull('cangkuquerenren.xingming')}
        , {title: '操作', toolbar: '#caozuo', fixed: 'right',width:180}
    ]];

    //初始化表格
    initTable("table", 'chengpinguanli/chengpinchukudengji/query_page', 'get', cols, table,"from");
        ...
        layer.open({
            ...
            , btn1: function (index, layero) {
                ...
                form.on('submit(form_add_submit)', function (data) {
                    ...
                    formData.heyuehao = {};
                    formData.heyuehao.id = chengpinCurrent_add.heyuehao.id;
                    encObject(formData);
                    $.ajax({
                        url: layui.setter.host + 'chengpinguanli/chengpinchukudengji/add',
        ...
        if (obj.event === 'del') {
            layer.confirm(
                ...
                {title: '删除提示'}, function (index) {
                    $.ajax({
                        url: layui.setter.host + 'chengpinguanli/chengpinchukudengji/delete',
                        type: 'get',
                        data: {'id': data.id},
                        success: function (data) {
                            ajaxSuccess(data, table);
            ...
            editI = layer.open({
                ...
                , btn1: function (editIndex, layero) {
                    ...
                    form.on('submit(form_edit_submit)', function (data) {
                        layer.confirm('确定要修改登记信息么?'
                            , function (i) {
                                var formData = data.field;
                                formData.heyuehao = {};
                                formData.heyuehao.id = chengpinCurrent_edit.heyuehao.id;
                                encObject(formData);
                                $.ajax({
                                    url: layui.setter.host + 'chengpinguanli/chengpinchukudengji/update',
                                    contentType: "application/json;charset=utf-8",
                                    type: 'POST',
                                    data: JSON.stringify(formData),
                                    success: function (data) {

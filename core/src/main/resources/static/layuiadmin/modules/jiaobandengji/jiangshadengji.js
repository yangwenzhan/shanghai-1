layui.define(['table', 'form', 'laydate','formSelects'], function(exports){
    ...
    var gongxu_jiangsha_id = 2;
    var gongxu_jiangsha = "浆纱";
    ...
    var cols =  [
        ...
        ,{field: 'pibuguige',sort:true, title: '坯布规格',width:180}
        ,{field: 'jitaihao',sort:true, title: '机台号',width:100}
        ,{field: 'changdu',sort:true, title: '经长m',width:90}
        ,{field: 'users',sort:true, title: '浆纱工',width:150}
        ,{field: 'beizhu',sort:true, title: '备注',width:150}
        ,{align: 'center',title: '操作',toolbar: '#barDemo',width:160, fixed: 'right'}
    ];
    cols = formatColumns(cols);

    initTable("table", 'jiaoban/jiangshadengji/findAll', 'get',[cols], table, "form");
    ...
    form.on('select(dj_ssgx)',function (data) {
        InitSelect('dj_jsg', null, 'common/findUser', 'get', {gxid:data.value}, 'ygxm', 'id',false);
        formSelects.render('addUser');
    });
    form.on('select(xg_dj_ssgx)',function (data) {
        InitSelect('xg_dj_jsg', null, 'common/findUser', 'get', {gxid:data.value}, 'ygxm', 'id',false);
    ...
    $('#add_btn').on('click',function(){
        ...
        //获取浆纱当前日期班次轮班
        var current = currentBanCiLunBan(gongxu_jiangsha);
        //fixme  begin 当前浆纱工序日期班次
        laydate.render({
            elem: '#dj_rq',
            type: 'date',
            value: current.riqi
        ...
        InitSelect('dj_jth', null, 'jichushuju/shebei/shebei/findAllSheBei', 'get', {gongxu:gongxu_jiangsha_id}, 'jitaihao', 'id',false);
        InitSelect('dj_ssgx', gongxu_jiangsha_id, 'common/findAllGX', 'get', {}, 'name', 'id',true);
        InitSelect('dj_jsg', null, 'common/findUser', 'get', {gxid:gongxu_jiangsha_id}, 'ygxm', 'id',false);
        formSelects.render('addUser');
        formSelects.value('addUser',[$('#id').val()]);

        layer.open({
            type: 1,
            title: ['浆纱交班产量登记', 'font-size:12px;'],
            ...
            yes: function(index, layero) {
                ...
                if(user_id_arr.length == 0){verifyWindow("请选择浆纱工!");return false;}
                ...
                layer.confirm('确定登记浆纱产量?'
                    ,function(i){
                        form.on('submit(form_add_submit)', function (data) {
                            ...
                            $.ajax({
                                url:layui.setter.host+'jiaoban/jiangshadengji/save',
                                type:'post',
    ...
    table.on('tool(table)',function(obj) {
        ...
        InitSelect('xg_dj_jth', data.jitaihao_id, 'jichushuju/shebei/shebei/findAllSheBei', 'get', {gongxu:gongxu_jiangsha_id}, 'jitaihao', 'id',false);
        InitSelect('xg_dj_ssgx', null, 'common/findAllGX', 'get', {}, 'name', 'id',true);
        InitSelect('xg_dj_jsg', data.users_id, 'common/findUser', 'get', {}, 'ygxm', 'id',false);
        formSelects.render('editUser');
        ...
        if(obj.event === 'edit'){
            layer.open({
                type: 1
                ,title: '编辑: '+data.riqi+data.banci+data.jitaihao+' 浆纱登记产量'
                ...
                ,btn1: function(index, layero) {
                    ...
                    layer.confirm('确定修改 '+data.riqi+data.banci+data.jitaihao+' 浆纱登记产量?'
                        ,function(i){
            ...
            var obj = {id:id};
            layer.confirm('确定删除 '+data.riqi+data.banci+data.jitaihao+' 浆纱登记产量?'
                ,function(i){
                    $.ajax({
                        url:layui.setter.host+'jiaoban/jiangshadengji/delete',
                        type:'post',
                        contentType:"application/json;charset=utf-8",

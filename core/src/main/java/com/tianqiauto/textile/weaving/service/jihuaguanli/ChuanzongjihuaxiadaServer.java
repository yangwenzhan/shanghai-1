...
public class ChuanzongjihuaxiadaServer {
    ...
    public Page<JiHua_ChuanZong_Main> findAll(JiHua_ChuanZong_Main jiHuaChuanZongMain, Pageable pageable) {
        Specification<JiHua_ChuanZong_Main> specification = (root, criteriaQuery, criteriaBuilder) -> {
            ModelUtil<JiHua_ChuanZong_Main> mu = new ModelUtil<>(jiHuaChuanZongMain,root);
            //开始日期和结束日期
            if(!StringUtils.isEmpty(jiHuaChuanZongMain.getKaishiriqi()) || !StringUtils.isEmpty(jiHuaChuanZongMain.getJieshuriqi())) {
                mu.addPred(criteriaBuilder.between(root.get("riqi"), jiHuaChuanZongMain.getKaishiriqi(),jiHuaChuanZongMain.getJieshuriqi()));
            }
            //班次
            ...
            //优先级
            Param youxianji = mu.initParam("youxianji.id");
        ...
        return jihuaChuanzongMainRepository.findAll(specification,pageable);
    }

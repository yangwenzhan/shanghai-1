 ...
 */
@RestController
@RequestMapping("jihuaguanli/chuanzongjihuaxiada")
public class ChuanzongjihuaxiadaController {


    @Autowired
    private ChuanzongjihuaxiadaServer chuanzongjihuaxiadaServer;

    @GetMapping("query_page")
    public Result findAll(JiHua_ChuanZong_Main jiHuaChuanZongMain, @PageableDefault( sort = { "youxianji.id","id" }, direction = Sort.Direction.DESC) Pageable pageable) {
        return Result.ok(chuanzongjihuaxiadaServer.findAll(jiHuaChuanZongMain,pageable));
    }
